...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "汇总 |staggered_mz|, S(pi,pi), average_abs_sz, "
            "并保存均值、标准差与 doping 图."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-prefix",
        type=str,
        default="defect_doping_average",
        help="输出文件名前缀, 默认 defect_doping_average",
    )
...
def collect_seed_rows_for_ndefect(data_root, lattice_size, ndefect):
    ...
    for seed_dir in seed_dirs:
        ...
        required_observables = ["staggered_mz", "S_pi_pi", "average_abs_sz"]
        ...
        seed_rows.append(
            {
                ...
                "average_abs_sz": float(block_table["average_abs_sz"]["Mean"]),
            }
        )
...
def build_average_rows(seed_rows):
    ...
    for ndefect in sorted(grouped_rows):
        ...
        abs_sz_mean, abs_sz_std = compute_mean_and_std([row["average_abs_sz"] for row in rows])
        ...
        average_rows.append(
            {
                ...
                "S_pi_pi_std": spi_std,
                "average_abs_sz_mean": abs_sz_mean,
                "average_abs_sz_std": abs_sz_std,
            }
        )
...
def plot_average_rows(output_path, average_rows):
    ...
    axes[2].errorbar(
        doping_values,
        [row["average_abs_sz_mean"] for row in average_rows],
        yerr=[row["average_abs_sz_std"] for row in average_rows],
        fmt="o-",
        capsize=4,
        linewidth=1.2,
        markersize=5,
