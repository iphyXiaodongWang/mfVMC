...
"""用途: 从 Emery `measure` 输出绘制 Cu-only Szz(qx, pi) 曲线.

输入文件:
- `block_binning_mean.json`, 需要包含 `Szzq_nx_ny` 格式的 observable key.

输出文件:
- 默认保存为输入 JSON 同目录下的 `emery_szz_qx_pi.png`.
"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="读取 Emery measure 的 block_binning_mean.json 并绘制 Szz(qx, pi)."
    )
    parser.add_argument(
        "input",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="输出图片路径. 默认保存到输入文件同目录的 emery_szz_qx_pi.png.",
    )
    parser.add_argument(
        "--lx",
        type=int,
        default=None,
        help="Cu cell 的 x 方向长度. 未给出时从 Szzq key 自动推断.",
    )
    parser.add_argument(
        "--ly",
        type=int,
        default=None,
        help="Cu cell 的 y 方向长度. 未给出时从 Szzq key 自动推断.",
    )
    parser.add_argument(
        "--dpi",
        type=int,
...
def extract_szz_qx_pi_line(
    values: List[SzzMomentumValue],
    lx: int,
    ly: int,
) -> Tuple[List[float], List[float]]:
    """用途: 提取 `qy=pi` 上的 `Szz(qx, pi)` 数据.
...
def plot_szz_qx_pi(
    ...
    参数:
    - `qx_over_pi`: List[float], 横轴 `qx / pi`.
    ...
    axis.set_title(r"Cu-only $S^{zz}(q_x,\pi)$")
    axis.set_xlim(min(qx_over_pi), max(qx_over_pi))
