...
function run_simulation(model, vwf, kernel, observables::Dict{Symbol, Function}, params::VMCParams)
    ...
    # --- 3. Warmup (热身) ---
    if is_root; println("[Driver] Starting Warmup ($(params.warmup_steps) sweeps)..."); end
    ...
    # --- 5. 采样循环 ---
    if is_root; println("[Driver] Starting Measurement Loop..."); end
    ...
    if is_root; println("[Driver] Sampling done in $(round(elapsed, digits=2)) s."); end
...
function run_sr_optimization(model, vwf, kernel, 
    ...
    for step in 1:sr_params.n_steps
        ...
        for _ in 1:vmc.warmup_steps
            for _ in 1:Nlat; mcmc_step!(runner, rng); end
