...
"""
用途: 将 `eta1` correction term 对单行 chain-rule 的 source-row 权重贡献累加到列表。
...
公式:
- `dU_b(i,sigma) += eta1_bf * sum_j t_ij D_i H_j dU_0(j,sigma)`。
"""
function add_backflow_correction_chain_rule_source_weights!(
    source_row_indices::AbstractVector{Int},
    source_row_weights::AbstractVector{T},
...
"""
用途: 将 `eta2` correction term 对单行 chain-rule 的 source-row 权重贡献累加到列表。
...
公式:
- `dU_b(i,sigma) += eta2_bf * sum_j t_ij
  n_i_sigma h_i_-sigma n_j_-sigma h_j_sigma dU_0(j,sigma)`。
"""
function add_backflow_correction_chain_rule_source_weights!(
    source_row_indices::AbstractVector{Int},
    source_row_weights::AbstractVector{T},
...
公式:
- `dU_b(i,sigma) += eta3_bf * sum_j t_ij D_i n_j_-sigma h_j_sigma dU_0(j,sigma)`。
"""
function add_backflow_correction_chain_rule_source_weights!(
    source_row_indices::AbstractVector{Int},
    source_row_weights::AbstractVector{T},
...
"""
用途: 将 split `eta4` correction term 对单行 chain-rule 的 source-row 权重贡献累加到列表。
...
公式:
- `dU_b(i,sigma) += eta4_bf * sum_j t_ij n_i_sigma h_i_-sigma H_j dU_0(j,sigma)`。
"""
function add_backflow_correction_chain_rule_source_weights!(
    source_row_indices::AbstractVector{Int},
    source_row_weights::AbstractVector{T},
    ...
    for bond_index in correction_term.outgoing_bond_indices_by_source[site_index]
        ...
        source_count = add_backflow_chain_rule_source_weight!(
            ...
            source_count,
            target_row,
            eta4_value * bond_amplitude * T(eta4_factor),
        )
    end
