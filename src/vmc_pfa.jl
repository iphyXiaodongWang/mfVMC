...
export vwf_pfa, VMCRunner, update_vwf_params!
...
function calc_ratio(vwf::vwf_pfa{T}, p::MoveProposal) where T
    # 如果 Proposal 无效
    if p.site1 == 0
        return one(T)
    end
...
function accept_move!(vwf::vwf_pfa{T}, p::MoveProposal, ratio::T) where T
...
function mcmc_step!(vwf::vwf_pfa{T}, kernel::AbstractMCMCKernel, rng::AbstractRNG; detailed_balance::Bool=false) where T
    cfg = vwf.sampler
...
function measure_green(vwf::vwf_pfa, i::Int, j::Int, spin_idx::Int8)
    ss = vwf.sampler
    if i == j
...
function measure_total_Sz(vwf::vwf_pfa)
    return sum(get_Sz(s) for s in vwf.sampler.state)
end

function measure_total_Hole(vwf::vwf_pfa)
    return sum(get_Hole(s) for s in vwf.sampler.state)
end

function measure_total_Doublon(vwf::vwf_pfa)
    return sum(get_Doublon(s) for s in vwf.sampler.state)
end

function measure_SzSz(vwf::vwf_pfa, i::Int, j::Int)
    return get_Sz(vwf.sampler.state[i]) * get_Sz(vwf.sampler.state[j])
end

function measure_SplusSminus(vwf::vwf_pfa, i::Int, j::Int)
    if i == j
        return 1.0
    end
    ss = vwf.sampler
...
"""
function measure_SplusSplus(vwf::vwf_pfa, i::Int, j::Int)
    if i == j
        return 0.0
    end
...
"""
function measure_SminusSminus(vwf::vwf_pfa, i::Int, j::Int)
    if i == j
        return 0.0
    end
...
"""
function measure_SxSx(vwf::vwf_pfa, i::Int, j::Int; conserve_sz=true)
    if i == j
        return 0.25
    end
...
function measure_SiSj(vwf::vwf_pfa, i::Int, j::Int)
