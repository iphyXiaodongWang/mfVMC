...
function config_Hubbard(N_sites::Int, N_up::Int, N_dn::Int)
    max_N = N_sites
...
function config_Heisenberg(N_sites::Int, N_up::Int)
    N_dn = N_sites - N_up
    ss = config_Hubbard(N_sites, N_up, N_dn)
    return ss
end

function copy_config(cfg)
...
function init_config_Hubbard!(ss; n_doublon::Union{Int, Nothing}=nothing)
    fill!(ss.state, HOLE)
...
function init_config_Heisenberg!(ss)
...
function count_choices_from_states(st1::Int8, st2::Int8, conserve_sz::Bool, is_same_site::Bool)
    ...
    # --- 1. Hop ---
    if check_hop(st1, st2, UP); c += 1; end # s1->s2 UP
    if check_hop(st2, st1, UP); c += 1; end # s2->s1 UP
    if check_hop(st1, st2, DN); c += 1; end # s1->s2 DN
    if check_hop(st2, st1, DN); c += 1; end # s2->s1 DN
    
    # --- 2. Exchange ---
    if check_exchange(st1, st2); c += 1; end
    
    # --- 3. Non-conserving Sz ---
    if !conserve_sz
        # Flip
        if check_flip(st1); c += 1; end
        if check_flip(st2); c += 1; end
        
        # Flip-Hop (Explicit check)
        # s1 -> s2
        if (st1 & UP)!=0 && (st2 & DN)==0; c+=1; end # s1 UP -> s2 DN
        if (st1 & DN)!=0 && (st2 & UP)==0; c+=1; end # s1 DN -> s2 UP
        # s2 -> s1
        if (st2 & UP)!=0 && (st1 & DN)==0; c+=1; end # s2 UP -> s1 DN
        if (st2 & DN)!=0 && (st1 & UP)==0; c+=1; end # s2 DN -> s1 UP
    end
...
function count_choices_heisenberg_from_states(st1::Int8, st2::Int8, conserve_sz::Bool, is_same_site::Bool)
    if is_same_site
        if !conserve_sz && check_flip(st1); return 1; else; return 0; end
    end
...
@inline function build_exchange(ss, s1::Int, s2::Int)
    ...
    if has_u1 && !has_u2
        # UP 从 s1 -> s2
        src = 2*(s1-1) + 1; tgt = 2*(s2-1) + 1
        id = ss.map_spin_to_id[src]
        if slot == 1; e1_id, e1_src, e1_tgt = id, src, tgt; else; e2_id, e2_src, e2_tgt = id, src, tgt; end
        slot += 1
    elseif !has_u1 && has_u2
        # UP 从 s2 -> s1
        src = 2*(s2-1) + 1; tgt = 2*(s1-1) + 1
        id = ss.map_spin_to_id[src]
        if slot == 1; e1_id, e1_src, e1_tgt = id, src, tgt; else; e2_id, e2_src, e2_tgt = id, src, tgt; end
        slot += 1
    end
    ...
    if has_d1 && !has_d2
        # DN 从 s1 -> s2
        src = 2*(s1-1) + 2; tgt = 2*(s2-1) + 2
        id = ss.map_spin_to_id[src]
        if slot == 1; e1_id, e1_src, e1_tgt = id, src, tgt; else; e2_id, e2_src, e2_tgt = id, src, tgt; end
        slot += 1
    elseif !has_d1 && has_d2
        # DN 从 s2 -> s1
        src = 2*(s2-1) + 2; tgt = 2*(s1-1) + 2
        id = ss.map_spin_to_id[src]
        if slot == 1; e1_id, e1_src, e1_tgt = id, src, tgt; else; e2_id, e2_src, e2_tgt = id, src, tgt; end
        slot += 1
    end
...
function propose_move_Hubbard!(ss, s1::Int, s2::Int, conserve_sz::Bool, rng::AbstractRNG)
    ...
    # --- 1. Hop Check ---
    if can_hop(ss, s1, s2, UP); push!(choices, 1); end
    if can_hop(ss, s2, s1, UP); push!(choices, 2); end
    if can_hop(ss, s1, s2, DN); push!(choices, 3); end
    if can_hop(ss, s2, s1, DN); push!(choices, 4); end

    # --- 2. Exchange Check ---
    if can_exchange(ss, s1, s2); push!(choices, 5); end

    # --- 3. Non-conserving Sz Check ---
    if !conserve_sz
        # Flip
        if can_flip(ss, s1); push!(choices, 6); end
        if can_flip(ss, s2); push!(choices, 7); end
        ...
        if s1 != s2
            ...
            if (st1 & UP)!=0 && (st2 & DN)==0; push!(choices, 8); end
            # DN -> UP (s1 has DN, s2 no UP)
            if (st1 & DN)!=0 && (st2 & UP)==0; push!(choices, 9); end
            
            # s2 -> s1
            # UP -> DN (s2 has UP, s1 no DN)
            if (st2 & UP)!=0 && (st1 & DN)==0; push!(choices, 10); end
            # DN -> UP (s2 has DN, s1 no UP)
            if (st2 & DN)!=0 && (st1 & UP)==0; push!(choices, 11); end
        end
    end
...
function propose_move_Heisenberg!(ss, s1::Int, s2::Int, conserve_sz::Bool, rng::AbstractRNG)
    ...
    if can_exchange(ss, s1, s2); push!(choices, 1); end
    
    if !conserve_sz
        if can_flip(ss, s1); push!(choices, 2); end
        if can_flip(ss, s2); push!(choices, 3); end
    end
...
@inline function rand_from_union(rng, list1, n1, list2, n2)
    tot = n1 + n2
    if tot == 0; return 0; end # Should not happen in standard Hubbard
    r = rand(rng, 1:tot)
    if r <= n1
...
function count_choices_Hubbard(ss, s1::Int, s2::Int, conserve_sz::Bool)
    ...
    if can_hop(ss, s1, s2, UP); c += 1; end
    if can_hop(ss, s2, s1, UP); c += 1; end
    if can_hop(ss, s1, s2, DN); c += 1; end
    if can_hop(ss, s2, s1, DN); c += 1; end
    
    if can_exchange(ss, s1, s2); c += 1; end
    
    if !conserve_sz
        if can_flip(ss, s1); c += 1; end
        if can_flip(ss, s2); c += 1; end
...
function propose_move(kernel::HubbardKernel, cfg::Configuration, rng::AbstractRNG)
    # 随机选择两个格点
...
function propose_move(kernel::HeisenbergKernel, cfg::Configuration, rng::AbstractRNG)
    # 随机选择两个格点
    # s1, s2 = rand(rng, 1:cfg.N_sites, 2)
    s1, s2 = rand_distinct_pair(rng, cfg.N_sites)
...
function check_consistency(ss)
    ...
    for site in 1:ss.N_sites
        st = ss.state[site]
        if (st & UP) != 0; state_up_count += 1; end
        if (st & DN) != 0; state_dn_count += 1; end
    end
